#if UNITY_EDITOR && METAVERSE_CLOUD_ENGINE
...
public class UnityEventCompactDrawer : PropertyDrawer
{
    ...
    internal const string kTargetAssemblyTypeName = "m_TargetAssemblyTypeName";
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        if (property == null ||
            ...
            property.serializedObject.FindProperty("m_Script") == null ||
            property.serializedObject.FindProperty("m_Script").objectReferenceValue == null)
        {
            EditorGUI.HelpBox(position, "Script is in an error state.", MessageType.Error);
            return;
        }

        if (EditorApplication.isCompiling)
        {
            EditorGUI.HelpBox(position, "Compiling...", MessageType.None);
            return;
        }
    ...
    protected virtual bool DrawListHeader(Rect rect, ReorderableList list)
    {
        ...
        // Header
        {
            EditorGUI.BeginProperty(foldoutRect, GUIContent.none, property);
    ...
    protected virtual void DrawEvent(Rect rect, int index, bool isActive, bool isFocused)
    {
        ...
        EditorGUI.BeginChangeCheck();
        {
            ...
            var originalTarget = listenerTarget.objectReferenceValue;
            EditorGUI.PropertyField(goRect, listenerTarget, GUIContent.none);
            if (EditorGUI.EndChangeCheck())
            {
                if (listenerTarget.objectReferenceValue != originalTarget)
                {
                    ...
                    if (!string.IsNullOrEmpty(argTypeNamePath) && type != null)
                    {
                        switch (newTarget)
                        {
                            case GameObject gameObject when type.IsSubclassOf(typeof(Component)):
                            {
                                var component = gameObject.GetComponent(type);
                                if (component != null)
                                {
                                    listenerTarget.objectReferenceValue = component;
                                    clearMethodName = false;
                                }
                            ...
                            default:
                            {
                                if (type.IsInstanceOfType(newTarget))
                                {
                                    listenerTarget.objectReferenceValue = newTarget;
                                    clearMethodName = false;
                                }
                                break;
                            }
        ...
        functionRect.xMax -= 25;

        Rect copyRect = new(rect)
        {
            xMin = functionRect.xMax + Spacing
        };
        GUIContent menuIcon = EditorGUIUtility.isProSkin ? EditorGUIUtility.IconContent("d_CustomTool") : EditorGUIUtility.IconContent("CustomTool");
        menuIcon.tooltip = "Right click for options.";
        EditorGUI.BeginProperty(copyRect, GUIContent.none, pListener);
        EditorGUI.EndProperty();

        EditorGUI.LabelField(copyRect, menuIcon);
    ...
    protected virtual void OnAddEvent(ReorderableList list)
    {
        ...
        {
            EditorUtility.DisplayDialog("Multi-Object Editing", "You cannot add events in multi object edit mode.", "Ok");
            return;
        }
