...
namespace MetaverseCloudEngine.Unity.Vehicles
{
    ...
    [AddComponentMenu(MetaverseConstants.MenuItems.MenuRootPath + "Vehicles/Land/Drivetrain/Vehicles - Wheel", 1)]

    // Class for the wheel
    public class Wheel : MonoBehaviour
    {
        private static PhysicMaterial _frictionlessMat;
        public static PhysicMaterial FrictionlessMat
        {
            get
            {
                return _frictionlessMat ??= new PhysicMaterial
                {
        ...
        private static readonly RaycastHit[] WheelHits = new RaycastHit[5];
        ...
        FloatingOriginObject origin;
        public PhysicMaterial detachedTireMaterial;
        public PhysicMaterial detachedRimMaterial;

        void Start() {
            tr = transform;
            origin = tr.GetTopmostParentComponent<FloatingOriginObject>();
            if (origin) origin.onShifted.AddListener(() => OnTeleported());
            ...
            if (tr.childCount > 0) {
                ...
                if (rim.childCount > 0) {
                    ...
                    {
                        var tireRen = tire.GetComponent<MeshRenderer>();
                        if (tireRen.enabled)
                        {
                            tireMat = new Material(tireRen.sharedMaterial);
                            tireRen.sharedMaterial = tireMat;   
                        }
                    }
                    ...
                    if (canDetach) {
                        detachedTire = new GameObject("Detached Tire")
                        {
                            transform =
                            {
                                parent = detachedWheel.transform,
                        ...
                        var detachTireRend = detachedTire.AddComponent<MeshRenderer>();
                ...
                if (Application.isPlaying) {
                    ...
                    if (generateHardCollider) {
                        ...
                        sphereColNew.layer = Physics.IgnoreRaycastLayer;
                        ...
                        sphereCol.sharedMaterial = FrictionlessMat;
                    }
        ...
        void FixedUpdate()
        {
            ...
            if (connected)
            {
                ...
                if (airLeakTime >= 0)
                {
                    ...
                    if (grounded)
                    {
                        airLeakTime += Mathf.Max(Mathf.Abs(currentRPM) * 0.001f, localVel.magnitude * 0.1f) * Time.timeScale * TimeValues.InverseFixedTimeFactor;
            ...
            if (!Application.isPlaying) {
                ...
                if (rimMat) {
                    if (rimMat.HasProperty("_EmissionColor")) {
                        // Make the rim glow
                        float targetGlow = connected && GroundSurfaceMaster.Instance.surfaceTypes[contactPoint.surfaceType].leaveSparks ? Mathf.Abs(F.MaxAbs(forwardSlip, sidewaysSlip)) : 0;
        ...
        void GetWheelContact() {
            ...
            int wheelHits = Physics.RaycastNonAlloc(suspensionParent.maxCompressPoint, suspensionParent.springDirection, WheelHits, castDist, Physics.DefaultRaycastLayers, QueryTriggerInteraction.Ignore);
            RaycastHit hit;
            int hitIndex = 0;
            bool validHit = false;
            ...
            if (validHit) {
                ...
                else if (curTerrain && curTerrain.enabled) {
        ...
        void ApplyFriction() {
            if (grounded) {
                ...
                float targetForceZ = sidewaysFrictionCurve.Evaluate(Mathf.Abs(sidewaysSlipFactor)) * -System.Math.Sign(sidewaysSlip) * (popped ? sidewaysRimFriction : sidewaysFriction) * sidewaysSlipDependenceFactor *
                    normalFrictionCurve.Evaluate(Mathf.Clamp01(Vector3.Dot(contactPoint.normal, Vector3.up))) *
