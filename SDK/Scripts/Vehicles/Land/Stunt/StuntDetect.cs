...
namespace MetaverseCloudEngine.Unity.Vehicles
{
    ...
    [AddComponentMenu(MetaverseConstants.MenuItems.MenuRootPath + "Vehicles/Land/Stunt/Vehicles - Stunt Detector", 1)]
    ...
    public class StuntDetect : MonoBehaviour
    {
        ...
        void DetectDrift() {
            endDriftTime = vp.groundedWheels > 0 ? (Mathf.Abs(vp.localVelocity.x) > 5 ? StuntManager.driftConnectDelayStatic : Mathf.Max(0, endDriftTime - Time.timeScale * TimeValues.InverseFixedTimeFactor)) : 0;
            drifting = endDriftTime > 0;

            if (drifting) {
                driftScore += (StuntManager.driftScoreRateStatic * Mathf.Abs(vp.localVelocity.x)) * Time.timeScale * TimeValues.InverseFixedTimeFactor;
                driftDist += vp.velMag * Time.fixedDeltaTime;
                driftString = "Drift: " + driftDist.ToString("n0") + " m";

                if (engine) {
                    engine.boost += (StuntManager.driftBoostAddStatic * Mathf.Abs(vp.localVelocity.x)) * Time.timeScale * 0.0002f * TimeValues.InverseFixedTimeFactor;
                }
            }
            else {
                score += driftScore;
        ...
        void DetectJump() {
            if (vp.groundedWheels == 0) {
                ...
                if (engine) {
                    engine.boost += StuntManager.jumpBoostAddStatic * Time.timeScale * 0.01f * TimeValues.InverseFixedTimeFactor;
                }
            }
            else {
                ...
                if (engine) {
                    engine.boost += (jumpDist + jumpTime) * StuntManager.jumpBoostAddStatic * Time.timeScale * 0.01f * TimeValues.InverseFixedTimeFactor;
                }
        ...
        void DetectFlips() {
            if (vp.groundedWheels == 0 && StuntManager.stuntsStatic != null) {
