...
namespace MetaverseCloudEngine.Unity.Vehicles
{
    ...
    [AddComponentMenu(MetaverseConstants.MenuItems.MenuRootPath + "Vehicles/Land/Damage/Vehicles - Vehicle Damage", 0)]
        ...
        void OnCollisionEnter(Collision col) {
            if (hitTime == 0 && col.relativeVelocity.sqrMagnitude * damageFactor > 1 && strength < 1) {
                ...
                foreach (ContactPoint curCol in col.contacts) {
                    if (tr.InverseTransformPoint(curCol.point).y > collisionIgnoreHeight) {
                        colsChecked++;
