...
namespace MetaverseCloudEngine.Unity.Vehicles
{
    ...
    [AddComponentMenu(MetaverseConstants.MenuItems.MenuRootPath + "Vehicles/Land/Effects/Vehicles - Tire Mark Creator", 0)]

    // Class for creating tire marks
    public class TireMarkCreate : MonoBehaviour
    {
        public const float TireMarkHeight = 0.02f;
        public const float TireMarkFadeTime = 10;
        public const float TireMarkGap = 0.1f;
        public const int TireMarkLength = 100;
        ...
        void Update() {
            ...
            if (w.grounded)
            {
                if (w.contactPoint.surfaceType < GroundSurfaceMaster.Instance.surfaceTypes.Length)
                    alwaysScrape = GroundSurfaceMaster.Instance.surfaceTypes[w.contactPoint.surfaceType].alwaysScrape ? slipThreshold + Mathf.Min(0.5f, Mathf.Abs(w.rawRPM * 0.001f)) : 0;
                else
                    alwaysScrape = 0;
            }
            else {
            ...
            if (w.grounded && (Mathf.Abs(F.MaxAbs(w.sidewaysSlip, w.forwardSlip)) > slipThreshold || alwaysScrape > 0) && w.connected) {
                ...
                if (curMark) {
                    ...
                    leftPoint = curMarkTr.InverseTransformPoint(w.contactPoint.point + pointDir * w.suspensionParent.flippedSideFactor * Mathf.Sign(w.rawRPM) + w.contactPoint.normal * TireMarkHeight);
                    rightPoint = curMarkTr.InverseTransformPoint(w.contactPoint.point - pointDir * w.suspensionParent.flippedSideFactor * Mathf.Sign(w.rawRPM) + w.contactPoint.normal * TireMarkHeight);
                }
            }
            ...
            if (curEdge < TireMarkLength && creatingMark) {
                UpdateMark();
            }
            else if (creatingMark) {
                EndMark();
            ...
            for (int i = 0; i < debrisParticles.Length; i++) {
                if (w.connected) {
                    if (i == w.contactPoint.surfaceType) {
                        if (GroundSurfaceMaster.Instance.surfaceTypes[w.contactPoint.surfaceType].leaveSparks && w.popped) {
        ...
        void StartMark() {
            ...
            verts = new Vector3[TireMarkLength * 2];
            tris = new int[TireMarkLength * 3];
            ...
            curEdge = 2;
            gapDelay = TireMarkGap;
        }

        // Update mark currently being generated
        void UpdateMark() {
            if (gapDelay == 0) {
                float alpha = (curEdge < TireMarkLength - 2 && curEdge > 5 ? 1 : 0) *
                    Random.Range(
        ...
        void EndMark() {
            ...
            curMark.GetComponent<TireMark>().fadeTime = TireMarkFadeTime;
