...
namespace MetaverseCloudEngine.Unity.Vehicles
{
    ...
    [AddComponentMenu(MetaverseConstants.MenuItems.MenuRootPath + "Vehicles/Land/Hover/Vehicles - Hover Wheel", 1)]
    ...
    public class HoverWheel : MonoBehaviour
    {
        ...
        private static readonly RaycastHit[] HoverWheelHits = new RaycastHit[5];
        ...
        void GetWheelContact() {
            ...
            int wheelHits = Physics.RaycastNonAlloc(tr.position, -upDir, HoverWheelHits, hoverDistance, Physics.DefaultRaycastLayers);
            bool validHit = false;
            float hitDist = Mathf.Infinity;

            // Loop through contact points to get the closest one
            for (int i = 0; i < wheelHits; i++) {
                RaycastHit curHit = HoverWheelHits[i];
