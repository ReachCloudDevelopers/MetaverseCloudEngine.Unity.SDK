...
namespace MetaverseCloudEngine.Unity.Vehicles
{
    ...
    [AddComponentMenu(MetaverseConstants.MenuItems.MenuRootPath + "Vehicles/Land/Vehicle Controllers/Vehicles - Steering Control", 2)]
    ...
    public class SteeringControl : MonoBehaviour
    {
        ...
        void FixedUpdate() {
            ...
            foreach (Suspension curSus in steeredWheels) {
                curSus.steerAngle = Mathf.Lerp(curSus.steerAngle, steerAmount * curSus.steerFactor * (curSus.steerEnabled ? 1 : 0) * (curSus.steerInverted ? -1 : 1), steerRate * TimeValues.InverseFixedTimeFactor * Time.timeScale);
            }
        }
