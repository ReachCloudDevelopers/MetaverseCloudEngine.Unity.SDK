...
namespace MetaverseCloudEngine.Unity.Vehicles
{
    ...
    [AddComponentMenu(MetaverseConstants.MenuItems.MenuRootPath + "Vehicles/Land/Vehicle Controllers/Vehicles - Vehicle Assist", 1)]
    ...
    public class VehicleAssist : MonoBehaviour
    {
        ...
        void Start() {
            ...
            initialAngularDrag = rb.GetAngularDamping();
        }
        ...
        void FixedUpdate() {
            if (vp.groundedWheels > 0) {
                ...
                angDragTime = 20;
                rb.SetAngularDamping(initialAngularDrag);
                ...
                if (angularDragOnJump) {
                    angDragTime = Mathf.Max(0, angDragTime - Time.timeScale * TimeValues.InverseFixedTimeFactor);
                    rb.SetAngularDamping(angDragTime > 0 && vp.upDot > 0.5 ? 10 : initialAngularDrag);
                }
            }
        ...
        void ApplySpinAssist() {
            ...
            float rightVelDot = Vector3.Dot(tr.right, rb.GetLinearVelocity().normalized);
        ...
        void RollOver() {
            ...
            if (vp.groundedWheels == 0 && vp.velMag < rollSpeedThreshold && vp.upDot < 0.8 && rollCheckDistance > 0) {
                if (Physics.Raycast(tr.position, vp.upDir, out rollHit, rollCheckDistance, Physics.DefaultRaycastLayers)
                    || Physics.Raycast(tr.position, vp.rightDir, out rollHit, rollCheckDistance, Physics.DefaultRaycastLayers)
                    || Physics.Raycast(tr.position, -vp.rightDir, out rollHit, rollCheckDistance, Physics.DefaultRaycastLayers)) {
                    rolledOver = true;
                }
                else {
        ...
        void ApplyDriftPush() {
            float pushFactor = (vp.accelAxisIsBrake ? vp.accelInput : vp.accelInput - vp.brakeInput) * Mathf.Abs(vp.localVelocity.x) * driftPush * groundedFactor * (1 - Mathf.Abs(Vector3.Dot(vp.forwardDir, rb.GetLinearVelocity().normalized)));
