...
namespace MetaverseCloudEngine.Unity.Vehicles
{
    ...
    [AddComponentMenu(MetaverseConstants.MenuItems.MenuRootPath + "Vehicles/Land/Vehicle Controllers/Vehicles - Vehicle Parent", 0)]
    ...
    public class VehicleParent : MonoBehaviour
    {
        ...
        void FixedUpdate() {
            ...
            // Check if reversing
            if (brakeIsReverse && brakeInput > 0.1f && localVelocity.z < 1 && burnout == 0) {
                reversing = true;
            }
        ...
        void OnCollisionEnter(Collision col) {
            if (col.contacts.Length > 0) {
                foreach (ContactPoint curCol in col.contacts) {
                    if (curCol.thisCollider.gameObject.layer != Physics.IgnoreRaycastLayer) {
