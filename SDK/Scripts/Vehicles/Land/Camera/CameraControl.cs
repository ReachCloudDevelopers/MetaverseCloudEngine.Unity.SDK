...
namespace MetaverseCloudEngine.Unity.Vehicles
{
    ...
    [AddComponentMenu(MetaverseConstants.MenuItems.MenuRootPath + "Vehicles/Land/Camera/Vehicles - Camera Control", 0)]
    ...
    public class CameraControl : MonoBehaviour
    {
        ...
        void FixedUpdate() {
            if (target && targetBody && target.gameObject.activeSelf) {
                ...
                targetUp = stayFlat ? Vector3.up : vp.norm.forward;
                lookDir = Vector3.Slerp(lookDir, (xInput == 0 && yInput == 0 ? Vector3.forward : new Vector3(xInput, 0, yInput).normalized), 0.1f * TimeValues.InverseFixedTimeFactor);
                smoothYRot = Mathf.Lerp(smoothYRot, targetBody.angularVelocity.y, 0.02f * TimeValues.InverseFixedTimeFactor);

                // Determine the upwards direction of the camera
                RaycastHit hit;
                if (Physics.Raycast(target.position, -targetUp, out hit, 1, castMask) && !stayFlat) {
                    upLook = Vector3.Lerp(upLook, (Vector3.Dot(hit.normal, targetUp) > 0.5 ? hit.normal : targetUp), 0.05f * TimeValues.InverseFixedTimeFactor);
                }
                else {
                    upLook = Vector3.Lerp(upLook, targetUp, 0.05f * TimeValues.InverseFixedTimeFactor);
                }

                // Calculate rotation and position variables
                forwardLook = Vector3.Lerp(forwardLook, targetForward, 0.05f * TimeValues.InverseFixedTimeFactor);
