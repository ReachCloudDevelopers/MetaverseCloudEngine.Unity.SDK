...
{
    public class YOLOPoseEstimation
    {
        Size input_size;
        ...
        public YOLOPoseEstimation(string modelFilepath, string classesFilepath, Size inputSize, float confThreshold = 0.25f, float nmsThreshold = 0.45f, int topK = 300, int backend = Dnn.DNN_BACKEND_OPENCV, int target = Dnn.DNN_TARGET_CPU)
        {
            // initialize
        ...
        public virtual List<Mat> infer(Mat image, List<Mat> results = null)
        {
            // cheack
            if (image.channels() != 3)
            {
